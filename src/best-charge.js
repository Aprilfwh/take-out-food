...
//返回最优的价格
  function final_price(order_list, promote) {
    ...
    for(var i=0;i<order_list.length;i++){
      ...
      for(var j=0; j<promote.items.length;j++){
        if(promote.items[j]===order_list[i].id){
    ...
    if(type1_price>type2_price){
      var promote_type2 = promote[1].type+"("+half_name.toString()+")，省"+half_price.toString() +"元";
  ...
  function output(order_list, price_result) {
    var result ='============= 订餐明细 =============\n';
    for(var i=0;i<order_list.length;i++){
      result += order_list[i].name + ' x ' + order_list[i].number.toString() + ' = '+order_list.total_price.toString()+'元\n';
    }
    result += '-----------------------------------\n';
